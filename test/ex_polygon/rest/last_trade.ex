defmodule ExPolygon.Rest.LastTradeTest do
  ...
  test ".query returns an ok tuple and last trade" do
    use_cassette "rest/last_trade/query_ok" do
      assert {:ok, last_trade} =
               ExPolygon.Rest.LastTradeOfTicker.query(
                 "AAPL",
                 @api_key
               )

      assert %ExPolygon.LastTrade{} = last_trade
      assert is_float(last_trade.price)
